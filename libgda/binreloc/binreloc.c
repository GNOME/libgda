...
_br_find_exe (GbrInitError *error)
{
	...
	if (error)
		*error = _GDA_GBR_INIT_ERROR_DISABLED;
	return NULL;
#else
	...
	if (path == NULL) {
		...
		if (error)
			*error = _GDA_GBR_INIT_ERROR_NOMEM;
		return NULL;
	}
	...
	if (f == NULL) {
		g_free (line);
		if (error)
			*error = _GDA_GBR_INIT_ERROR_OPEN_MAPS;
		return NULL;
	}
	...
	if (result == NULL) {
		...
		g_free (line);
		if (error)
			*error = _GDA_GBR_INIT_ERROR_READ_MAPS;
		return NULL;
	}
	...
	if (buf_size <= 0) {
		...
		g_free (line);
		if (error)
			*error = _GDA_GBR_INIT_ERROR_INVALID_MAPS;
		return NULL;
	}
 ...
 */
gboolean
_gda_gbr_init_lib (GError **error)
{
	GbrInitError errcode;
 ...
 */
gchar *
_gda_gbr_find_exe (const gchar *default_exe)
{
	if (exe == NULL) {
 ...
 */
gchar *
_gda_gbr_find_exe_dir (const gchar *default_dir)
{
	if (exe == NULL) {
 ...
 */
gchar *
_gda_gbr_find_prefix (const gchar *default_prefix)
{
	gchar *dir1, *dir2;
 ...
 */
gchar *
_gda_gbr_find_bin_dir (const gchar *default_bin_dir)
{
	gchar *prefix, *dir;

	prefix = _gda_gbr_find_prefix (NULL);
	if (prefix == NULL) {
		/* BinReloc not initialized. */
 ...
 */
gchar *
_gda_gbr_find_sbin_dir (const gchar *default_sbin_dir)
{
	gchar *prefix, *dir;
 ...
 */
gchar *
_gda_gbr_find_data_dir (const gchar *default_data_dir)
{
	gchar *prefix, *dir;
 ...
 */
gchar *
_gda_gbr_find_locale_dir (const gchar *default_locale_dir)
{
	gchar *data_dir, *dir;

	data_dir = _gda_gbr_find_data_dir (NULL);
	if (data_dir == NULL) {
		/* BinReloc not initialized. */
 ...
 */
gchar *
_gda_gbr_find_lib_dir (const gchar *default_lib_dir)
{
	gchar *prefix, *dir;
 ...
 */
gchar *
_gda_gbr_find_etc_dir (const gchar *default_etc_dir)
{
	gchar *prefix, *dir;
