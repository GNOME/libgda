...
#define SQL_OPERAND(x) ((SqlOperand*)(x))
	...
	case GDA_SQL_OPERATOR_TYPE_GLOB:
		...
		break;
	case GDA_SQL_OPERATOR_TYPE_NE:
		str = g_strdup_printf ("%s != %s", SQL_OPERAND (sql_list->data)->sql, SQL_OPERAND (sql_list->next->data)->sql);
		break;
	case GDA_SQL_OPERATOR_TYPE_ISNOT:
		str = g_strdup_printf ("%s IS NOT %s", SQL_OPERAND (sql_list->data)->sql, SQL_OPERAND (sql_list->next->data)->sql);
		break;
	default:
		g_assert_not_reached ();
		break;
