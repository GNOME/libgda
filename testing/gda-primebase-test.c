...
/* ------------------------------------------------------------------------- */
/* List all primebase dbms
/* ------------------------------------------------------------------------- */
void
list_dbms (Gda_Connection* cnc)
{
  Gda_Recordset* rs;
  Gda_Command* cmd;
  Gda_Field* field;
  ...
  gda_command_set_text(cmd, "DESCRIBE DBMS; PRINTALL;");
  g_print ("\nopening dbms...\n");
  rs = gda_command_execute(cmd, &affected, 0);
  if (!rs) die (cnc);
  ...
  gda_command_set_connection(cmd, NULL);
