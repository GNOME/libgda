...
void *
memsql_strdup_printf_raw(char *funcname, int linenum, char *file,const char *fmt,...)
	{
	void *retptr;
	gchar *tmpstr;
	va_list args;
...
void *
memsql_realloc_raw(char *funcname, int linenum, char *file,void *mem,  int count)
   {
   void *new_mem;
   memsql_pre_info *pre;
...
memsql_display(void)
   ...
   for (cur = memsql_list; cur != NULL; cur = cur->next)
      ...
      if (!pre->freed)
         {
#endif
         memsql_error("%s: %d: %s() %dbytes still alloced", pre->file, pre->line, pre->name, pre->size);
...
char *
memsql_strappend_raw(char *funcname,int linenum, char *file,  char *string1, char *string2)
   {
   int len = 0;
   char *retval;
...
char *
memsql_strappend_free_raw(char *funcname,int linenum, char *file,  char *str1, char *str2)
   {
   char *retval;
