                               ...
                               const gchar *password)
{
  ...
  hstmt = PBIConnect(&s_id, pcnc->host, PB_DATA_SERVER, PB_TCP, NULL,
                     t_user, t_pass, database);
  pcnc->sid = s_id;
  if (hstmt == PB_ERROR) {
    gda_log_message(_("Could not connect to '%s@%s' with connparm '%s'"),
                    t_user, pcnc->host, pcnc->connparm);
    return -1;
  }
