...
gda_primebase_recordset_move_next (GdaServerRecordset * recset)
{
	...
	if (!prset->initialized) {
		...
		while ((pcnc->state =
			CLGetItem (pcnc->sid, AW_DEFAULT, &prset->type,
			...
			if (pcnc->state == A_ERROR) {
				...
				pcnc->err =
					gda_primebase_get_error (pcnc->sid,
								 TRUE);
				gda_primebase_free_error (pcnc->err);
				return -1;
			}
			...
			pcnc->state =
				CLGetItem (pcnc->sid, AW_DEFAULT,
					   &prset->type, &prset->len,
					   &prset->places, &prset->flags,
					   prset->buffer);
			gda_primebase_update_cell (field, prset,
						   (gpointer) prset->buffer);
		...
		while ((pcnc->state =
			...
			// Update field
			field = g_list_nth (gda_server_recordset_get_fields
					    (recset), col);
			if (field
			    && (gda_server_field_get_sql_type (field) ==
				prset->type)) {
				prset->type = A_ANYTYPE;
				prset->buffer =
					g_new0 (gchar, prset->len + 1);
				// FIXME: Get items of more then MAX_DALSIZE
				pcnc->state =
					CLGetItem (pcnc->sid, AW_DEFAULT,
						   &prset->type, &prset->len,
