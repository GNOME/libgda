			  ...
			  const gchar * user, const gchar * password)
{
	...
	if (ld_cnc) {
		...
		while (ptr_s && *ptr_s) {
			...
			if (strncasecmp (ptr_s, "HOST", strlen ("HOST")) == 0)
			...
			else if (strncasecmp (ptr_s, "PORT", strlen ("PORT"))
				 == 0)
				port = get_value (ptr_s);
			else if (strncasecmp
				 (ptr_s, "AUTHENTICATION",
				...
				if (!strncasecmp
					...
					auth_method = LDAP_AUTH_KRBV42;
					 ...
					 (auth, "SIMPLE", strlen ("SIMPLE")))
					auth_method = LDAP_AUTH_SIMPLE;
				else
					auth_method = LDAP_AUTH_NONE;
		...
		ld_cnc->ldap =
			ldap_init (host, port ? atoi (port) : LDAP_PORT);
		g_free ((gpointer) host);
		g_free ((gpointer) port);
		if (ld_cnc->ldap) {
			/* bind to the LDAP server */
			if (ldap_bind_s
			    (ld_cnc->ldap, user, password,
