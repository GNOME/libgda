...
//      gda_server_error_make(error, 0, cnc, "connection seems to be dead");
  ...
  if ((tcnc->ret = ct_command(tcmd->cmd, CS_LANG_CMD, sql_cmd, CS_NULLTERM,
                              CS_UNUSED
                             )
      ) != CS_SUCCEED
     ) {
  ...
  */
  while ((tcnc->ret = ct_results(tcmd->cmd, &result_type)
    ...
    case CS_CURSOR_RESULT:
      gda_tds_init_recset_fields(error, recset, trecset, result_type);
      if (recset) {
        if (affected) {
    ...
    default:
      ...
      if (trecset && !trecset->data) {
        ct_cancel(NULL, tcmd->cmd, CS_CANCEL_CURRENT);
      } else {
