...
get_table_fields (GdaConnection *cnc, GdaXmlDatabase *xmldb, GdaParameterList *params)
{
	...
	for (i = 0; i < cols; i++) {
		...
		value_list = g_list_append (value_list, gda_value_new_string (gda_column_get_name (fa)));
		value_list = g_list_append (value_list, gda_value_new_string (gda_type_to_string (gda_column_get_gdatype (fa))));
		value_list = g_list_append (value_list, gda_value_new_integer (gda_column_get_defined_size (fa)));
		value_list = g_list_append (value_list, gda_value_new_integer (gda_column_get_scale (fa)));
