...
// $Log$
// Revision 1.1  2000/08/10 09:32:39  rodrigo
// Initial revision
//
  ...
  for (i = 0; i < srec->colscnt; i++) {
	...
	if (srec->data[i].data) {
	  ...
	  case CS_DATETIME_TYPE:
	    gda_sybase_field_set_datetime(field,
					  (CS_DATETIME *) srec->data[i].data);
	    break;
	  case CS_DATETIME4_TYPE:
	    gda_sybase_field_set_datetime4(field,
					   (CS_DATETIME4 *) srec->data[i].data);
	    break;
	  default:
	    // try cs_convert data to CS_CHAR_TYPE
	    gda_sybase_field_set_general(field, &srec->data[i],
					 srec->scnc);
	    break;
	  }
	} else {
			       ...
			       CS_RETCODE result_type)
{
  ...
  for (colnr = 0; colnr < srecset->colscnt; colnr++) {
    ...
    gda_log_message(_("ct_describe(%d): '%s', %d (%s)"), colnr + 1,
		    ...
		    sybase_get_type_name(datafmt[colnr].datatype));
#endif
