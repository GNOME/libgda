...
gda_sybase_connection_close(GdaServerConnection *cnc)
{
  ...
  // Close connection
  if (scnc->cnc) {
    if (SYB_CHK((CS_INT *) &scnc->ret, ct_close(scnc->cnc, CS_UNUSED),
		NULL, NULL, cnc, NULL
		) != CS_SUCCEED
	) {
  ...
  if (scnc->ctx) {
    ...
    if ((scnc->ret = ct_exit(scnc->ctx, CS_UNUSED)) != CS_SUCCEED) {
      gda_sybase_cleanup(scnc, scnc->ret, "ct_exit failed");
      return;
    }
    
    if ((scnc->ret = cs_ctx_drop(scnc->ctx)) != CS_SUCCEED) {
      gda_sybase_cleanup(scnc, scnc->ret, "cs_ctx_drop failed");
      return;
    }
    scnc->ctx = (CS_CONTEXT *) NULL;
  }
...
gda_sybase_connection_reopen(GdaServerConnection *cnc)
{
  ...
  */
  ...
  if (SYB_CHK((CS_INT *) &scnc->ret,
    ...
    gda_sybase_cleanup(scnc, scnc->ret, "Could not set application name");
    return FALSE;
  }
  if (SYB_CHK((CS_INT *) &scnc->ret,
    ...
    gda_sybase_cleanup(scnc, scnc->ret, "Could not set user name");
    return FALSE;
  }
  if (SYB_CHK((CS_INT *) &scnc->ret,
    ...
    gda_sybase_cleanup(scnc, scnc->ret, "Could not set password");
    return FALSE;
  }
                                      ...
                                      const gchar *database)
{
  ...
  // Send the command
  if (SYB_CHK(NULL, ct_send(cmd), NULL, NULL, cnc, NULL) != CS_SUCCEED) {
...
void
gda_sybase_cleanup(sybase_Connection *scnc, CS_RETCODE ret, const gchar *msg)
{
  g_return_if_fail(scnc != NULL);
