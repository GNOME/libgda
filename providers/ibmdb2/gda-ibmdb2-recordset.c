...
static GdaRow       *gda_ibmdb2_recordset_get_row 	(GdaDataModelRow *model, gint rownum, GError **error);
...
static GdaRow *
gda_ibmdb2_recordset_get_row (GdaDataModelRow *model, gint row, GError **error)
{
...
gda_ibmdb2_recordset_get_value_at (GdaDataModelRow *model, gint col, gint row)
{
	...
	g_return_val_if_fail (recset->priv != NULL, 0);
        ...
        fields = gda_ibmdb2_recordset_get_row (model, row, NULL);
...
static gint
gda_ibmdb2_recordset_get_n_rows (GdaDataModelRow *model)
{
	GdaIBMDB2Recordset *recset = (GdaIBMDB2Recordset *) model;
...
static gint
gda_ibmdb2_recordset_get_n_columns (GdaDataModelRow *model)
{
	GdaIBMDB2Recordset *recset = (GdaIBMDB2Recordset *) model;
...
static void
gda_ibmdb2_recordset_describe (GdaDataModelRow *model, gint col)
{
	GdaIBMDB2Recordset *recset = (GdaIBMDB2Recordset *) model;
	...
	field = (GdaIBMDB2Field*) g_ptr_array_index (recset->priv->columns, col);
...
gda_ibmdb2_recordset_new (GdaConnection *cnc, SQLHANDLE hstmt)
{
	...
	for (i = 0; i < ncols; i++) {
		...
		gda_ibmdb2_recordset_describe (GDA_DATA_MODEL_ROW (recset), i);
	}
