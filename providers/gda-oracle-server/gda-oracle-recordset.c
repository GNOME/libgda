...
fill_field_values (GdaServerRecordset * recset)
{
	...
	/* traverse all fields */
	for (node = g_list_first (recset->fields); node != NULL;
		...
		ora_field =
			(ORACLE_Field *)
			gda_server_field_get_user_data (field);
		if (ora_field) {
			...
			if (ora_field->real_value) {
				...
				default:
					gda_log_error (_
						       ("Unknown ORACLE data type in fill_field_values"));
					break;
				}
			}
			else {
				gda_server_field_set_actual_length (field, 0);
				memset (&field->value->_u, 0,
					sizeof (field->value->_u));
			}
		}
	}
	return TRUE;
...
gda_oracle_recordset_new (GdaServerRecordset * recset)
{
	...
	if (cnc) {
		...
		gda_log_message (_
				 ("recordset object being created without associated connection"));
	return TRUE;
}
...
gda_oracle_recordset_free (GdaServerRecordset * recset)
{
	...
	if (ora_recset) {
		...
		/* free all fields */
		for (node = g_list_first (recset->fields);
