...
init_recordset_fields (GdaServerRecordset * recset,
		       ...
		       ORACLE_Command * ora_cmd, GdaError * error)
{
	...
	if (ora_cmd->stmt_type == OCI_STMT_SELECT) {
		...
		while (param_status == OCI_SUCCESS) {
			...
			OCIAttrGet ((dvoid *) param,
				    ...
				    &pgchar_dummy,
				    (ub4) & col_name_len,
				    OCI_ATTR_NAME, ora_recset->ora_cnc->herr);
			...
			ora_field->real_value =
				g_malloc0 (field->defined_length);
			ora_field->hdef = NULL;
...
gda_oracle_command_new (GdaServerCommand * cmd)
{
	...
	if (cnc) {
		...
		if (OCI_SUCCESS !=
		    OCIHandleAlloc ((CONST dvoid *) ora_cmd->ora_cnc->henv,
				    (dvoid **) & ora_cmd->hstmt,
		...
		gda_log_message (_
				 ("command object being created without associated connection"));
	return TRUE;
}
			    ...
			    gulong * affected, gulong options)
{
	...
	recset = gda_server_recordset_new (gda_server_command_get_connection
					   (cmd));
	if (recset) {
		ORACLE_Command *ora_cmd;
		ORACLE_Recordset *ora_recset;
		...
		ora_cmd =
			(ORACLE_Command *)
			gda_server_command_get_user_data (cmd);
		ora_recset =
			(ORACLE_Recordset *)
		...
		if (ora_cmd && ora_recset) {
			...
			case GDA_COMMAND_TYPE_TABLE:{
					gchar *tmp =
						...
						("SELECT * FROM %s",
						 ...
						 (cmd));
					gda_server_command_set_text (cmd,
								     tmp);
					g_free ((gpointer) tmp);
				}
			case GDA_COMMAND_TYPE_TEXT:
				...
				if (OCI_SUCCESS ==
					...
					if (OCI_SUCCESS ==
					    OCIStmtExecute (ora_cmd->ora_cnc->
							    hservice,
							    ora_cmd->hstmt,
							    ora_cmd->ora_cnc->
							    herr,
							    (ub4) ((OCI_STMT_SELECT == ora_cmd->stmt_type) ? 0 : 1), (ub4) 0, (CONST OCISnapshot *) NULL, (OCISnapshot *) NULL, OCI_DEFAULT)) {
						ora_recset->ora_cnc =
							ora_cmd->ora_cnc;
						if (OCI_STMT_SELECT ==
						    ora_cmd->stmt_type) {
...
gda_oracle_command_free (GdaServerCommand * cmd)
{
	...
	if (ora_cmd) {
		...
		if (ora_cmd->hstmt != NULL)
			OCIHandleFree ((dvoid *) ora_cmd->hstmt,
				       OCI_HTYPE_STMT);
