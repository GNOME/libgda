...
fill_field_values (GdaServerRecordset *recset, MYSQL_Recordset *mysql_recset)
{
	...
	if (mysql_recset->mysql_res) {
		...
		mysql_recset->array = mysql_recset->fix_fields(mysql_recset, mysql_recset->array);
	      ...
	      rowlength = GdaBuiltin_Result_get_nbfields(mysql_recset->btin_res);
		...
		if (node) {
			...
			if (mysql_recset->mysql_res)
			        thevalue = mysql_recset->array[fieldidx];
  			        ...
  			        thevalue = GdaBuiltin_Result_get_value(mysql_recset->btin_res, row, fieldidx);
			if (thevalue) {
				GdaServerField* field = (GdaServerField *) node->data;
				switch (gda_server_field_get_sql_type(field)) {
				case FIELD_TYPE_TINY :
				case FIELD_TYPE_SHORT :
					gda_server_field_set_smallint(field, atoi(thevalue));
					break;
				case FIELD_TYPE_LONG :
					gda_server_field_set_integer(field, atoi(thevalue));
					break;
				case FIELD_TYPE_DECIMAL :
				case FIELD_TYPE_FLOAT :
					gda_server_field_set_single(field, atof(thevalue));
					break;
				case FIELD_TYPE_DOUBLE :
					gda_server_field_set_double(field, atof(thevalue));
					break;
				case FIELD_TYPE_DATE :
					stm = str_to_tmstruct_date2(thevalue);
					if (stm) {
						GDate *date;
...
gda_mysql_recordset_move_next (GdaServerRecordset *recset)
{
	...
	if (mysql_recset) {
			...
			if (mysql_recset->array) {
		        ...
		        gint ntuples;
			ntuples = GdaBuiltin_Result_get_nbtuples(mysql_recset->btin_res);
